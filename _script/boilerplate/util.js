...
function createUpload(config){
    ...
    var buttonText = config.buttonText || '<i class="fa fa-upload"></i> Upload';
    var buttonClass = config.buttonClass || "textlink action add";
    ...
    if (config.onDone){
        addFile = config.onDone;
    }else{
        if (name) {
            input = createTextarea('hidden',name,null,value);
            listView = createDiv('listView');
        }
    ...
    var init = createDiv(buttonClass);
    init.innerHTML = buttonText;
    ...
    if (listView) result.appendChild(listView);
    if (input) result.appendChild(input);

    return result;
}

function createSearchBox(className,placeHolder,onSearch){
    className = className || "";
    var searchbox = createDiv("searchbox " + className);
    var searchInput = createInput("searchinput",null,null,"");
    searchInput.placeholder = placeHolder || "Search";
    searchInput.type = "search";
    searchInput.onchange = onSearch || function(){};
...
function concatStringAdd(s,value){
    s = concatStringSort(s);
    var _s = "," + s.toLowerCase() + ",";
    if (_s.indexOf("," + value.toLowerCase() + ",") < 0) {
        s = s + "," + value;
        s = concatStringSort(s);
    }
...
function concatStringRemove(s,value){
    var arr = s.split(",");
    var result = [];
    var _value = value.trim().toLowerCase();
    for (var i = 0; i < arr.length; i++) {
        arr[i] = arr[i].trim();
        if (arr[i] != "" && arr[i].toLowerCase() != _value) result.push(arr[i]);
    }
    result.sort(function(a,b){
        return a > b;
    });
